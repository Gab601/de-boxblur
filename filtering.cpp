...
Image deBoxBlurX(const Image &im, int k, bool clamp) {
  // Attempt to deconvolve a box blur of size k by k in the X direction
  ...
  for (int c = 0; c < im.channels(); c++) {
    for (int y = 0; y < im.height(); y++) {
      ...
      float total = 0;
      for (int x = 0; x < int((k-1.0f)/2.0f); x++) {
        float val = 0;
        out(x, y, c) = val;
        sums[x%k] += val;
        total += val;
      }
      for (int x = int((k-1.0f)/2.0f); x < im.width(); x++) {
        float curSum = 0;
        for (int dx = -k+1; dx < 0; dx++) {
          curSum += out.smartAccessor(x+dx, y, c, clamp);
        }
        float val = im.smartAccessor(x - int((k-1.0f)/2.0f), y, c, clamp) * k - curSum;
        out(x, y, c) = val;
        sums[x%k] += val;
        total += val;
      }
      ...
      for (int x = 0; x < im.width(); x++) {
        out(x, y, c) += total / im.width() - sums[x%k] / (im.width() / k + (im.width() % k > x%k));
      }
    }
  }
...
Image deBoxBlurY(const Image &im, int k, bool clamp) {
  // Attempt to deconvolve a box blur of size k by k in the Y direction
  Image flipped = flip(im);
  Image deblurred = deBoxBlurX(flipped, k, clamp);
  return flip(deblurred);
}

Image deBoxBlur(const Image &im, int k, bool clamp) {
  // Attempt to deconvolve a box blur of size k by k
  Image intermediate = deBoxBlurX(im, k, clamp);
  return deBoxBlurY(intermediate, k, clamp);
}
